 # tests/testthat/test-check_covariate_missingness.R
...
test_that("explicit columns override auto-selection", {
  ...
  result <- check_covariate_missingness(dat, X_b, X_c)
...
test_that("auto-selects X_ columns when none passed", {
  ...
  result <- check_covariate_missingness(dat)
...
test_that("returns NULL with warning if no variables selected", {
  ...
  expect_warning(result <- check_covariate_missingness(dat), "No variables selected")
  expect_null(result)
})
